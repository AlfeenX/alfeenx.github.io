// ===== PRELOADER WITH SMOOTH SLIDE DOWN ANIMATION =====
(function () {
  const preloader = document.getElementById("preloader");
  const preloaderPanel = document.querySelector(".preloader-panel");
  const preloaderContent = document.querySelector(".preloader-content");

  if (!preloader || !preloaderPanel || !preloaderContent) return;

  void preloader.offsetHeight;

  setTimeout(() => {
    preloaderContent.classList.add("visible");
  }, 100);
  ...
  function startExitAnimation() {
    ...
    setTimeout(() => {
      requestAnimationFrame(() => {
        preloaderPanel.classList.add("slide-down");
      });
  ...
  if (document.readyState === "complete") {
    ...
    window.addEventListener("load", startExitAnimation);
  }
})();
